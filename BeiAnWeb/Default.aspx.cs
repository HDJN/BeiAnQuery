...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(sValue.Value)
    && !string.IsNullOrEmpty(sType.Value)
    && !string.IsNullOrEmpty(sName.Value))
            {

                GetBeiAnResult(sName.Value, sType.Value, sValue.Value);
            }
            else
            {
